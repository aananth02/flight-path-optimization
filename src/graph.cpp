...
vector<Edge> Graph::getEdges() const
{
    ...
    for (size_t i = 0; i < ver_index.size(); i++)
    {
        ...
        {
            if (adjacency_matrix[i][j].first > 0)
                ret.push_back(getEdge(ver_index[i], ver_index[j])); // only gives positive edges (source->destination)
        }
    }
...
{
    if (find(ver_index.begin(), ver_index.end(), v) == ver_index.end())
        return Vertex();
...
bool Graph::insertEdge(Vertex source, Vertex destination, float weight, string label)
{
    ...
    if (directed)
        adjacency_matrix[getVertexIdx(destination)][getVertexIdx(source)] = pair<float, string>(weight * -1, label);
